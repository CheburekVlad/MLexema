kmeans_analysis = function(input_data,clusters=2,nstart = 5,max.iter= 15,voiName){
  ...
  conf = data.frame(data$Patients,fittedV,diag_bin,diag)
...
plot_placeholder = function(){
  ggplot() + theme_void() + 
    annotate("text", x = 0.5, y =0.5, label = "Courbe n'est pas disponible pour Kmeans", size = 7, hjust = 0.5, vjust = 0.5, color = "red")
}