...
pathEstelle = 'eczema_data.xlsx'
pathTim = 'dataset_RF_sans_barx2.xlsx'

file = read_xlsx(file.path(pathTim))

pca <- prcomp(file[, 2:13], center = TRUE, scale. = TRUE) # Effectuez l'analyse PCA
pca
donneesPca <- data.frame(pca$x[, 1:2], diagnostique = file$Diagnosis) # Créez un dataframe avec les deux premières composantes principales et la variable diagnostique
donneesPca
...
# Charger les données
data <- read_xlsx(file.path(pathTim))
